...
data _null_;
	call symput ('exportMLA1', 
		'\\mktg-APP01\E\Production\MLA\MLA-Input files TO WEBSITE\PB_HOTLIST_20220502p1.txt');
	call symput ('exportMLA2', 
		'\\mktg-APP01\E\Production\MLA\MLA-Input files TO WEBSITE\PB_HOTLIST_20220502p2.txt');
run;
	...
	*** SUBSET `dw.vw_loan_NLS` USING RELEVANT VARIABLES --------- ***;
	SET dw.vw_loan_NLS(
		KEEP = PURCD CIFNO BRACCTNO XNO_AVAILCREDIT XNO_TDUEPOFF ID 
			   ...
			   XNO_TDUEPOFF CURBAL CONPROFILE1 Acctrefno XNO_TDueTBal);
	...
	*** NULLS FROM `BNKRPTDATE`. --------------------------------- ***;
	WHERE CIFNO NE "" & 
		  ...
		  OWNST IN ("NC","VA","NM","SC","OK","TX", "AL", "GA", "TN", 
					"MO", "WI", "IL", "UT", "MS") /*&
		  OWNBR IN ("0533", "0536", "0525", "0541", "0587", "0567", 
					"0578", "0902", "0901", "0911", "1010", "1003", 
					"0705", "0831", "1211", "0908", "0814", "0804", 
					"0876", "0812", "0828", "0824", "0805", "0811", 
					"0820", "0893", "0807", "0889", "0801", "0829", 
					"0887", "0882")*/;
	...
	*** SUBSET `dw.vw_loan` USING RELEVANT VARIABLES ------------- ***;
	SET dw.vw_loan(
		KEEP = PURCD BRACCTNO XNO_AVAILCREDIT XNO_TDUEPOFF ID OWNBR
			   ...
			   PLDATE PLAMT BNKRPTDATE BNKRPTCHAPTER DATEPAIDLAST 
			   APRATE CRSCORE NETLOANAMOUNT XNO_AVAILCREDIT 
			   XNO_TDUEPOFF CURBAL CONPROFILE1 XNO_TDueTBal);
...
DATA MERGED_L_B2; /* MERGE PULL WITH FLAGS */
...
**********************************************************************;
**********************************************************************;
data partial_chgoffs;
	set '\\rmc.local\dfsroot\Dept\Marketing\Analytics\SNAPSHOTS\partial_chgoffs.sas7bdat';
run;

DATA partial_chgoffs;
	SET partial_chgoffs(
		KEEP = BRACCTNO CIFNO);
	CIFNUM = input(CIFNO, 10.);
    drop CIFNO;
   	rename CIFNUM=CIFNO;
	PARTCHGOFFS_FLAG = "X";
RUN;
...
DATA MERGED_L_B2_A;
	MERGE MERGED_L_B2_B(IN = x) partial_chgoffs;
	BY CIFNO;
	IF x;
RUN;
**********************************************************************;
...
DATA MERGED_L_B2;
	MERGE MERGED_L_B2_A(IN = x) loanacct_detail;
	BY acctrefno;
	IF x;
	*Payoff_Amount = XNO_TDueTBal;
	*Available_Credit = XNO_AvailCredit;
RUN;
...
PROC IMPORT 
	DATAFILE = "\\server-lcp\LiveCheckService\DNHCustomers\DNHFile-04-28-2022-06-26.xlsx" 
		OUT = DNS DBMS = XLSX;
	SHEET = "DNS";
RUN;
...
PROC SUMMARY 
	...
	CLASS BRACCTNO;
	VAR DELQ1-DELQ12 RECENT3 RECENT6 LAST12 FIRST6 LAST12_60 CD60 CD30;
...
DATA MERGED_L_B2; /* FLAG FOR BAD DLQ */
	SET MERGED_L_B2;
	*IF AGE = 1 THEN DO;
		*IF CD > 2 THEN DLQ_FLAG = "X";
	*END;
	IF TIMES30 = . THEN DLQ_FLAG = "X";
	
	IF RECENT3 >= 1 OR LAST12 >= 2 OR LAST12_60 >= 1 
		THEN DLQ_FLAG = "X";
	IF DELQ1 NOT IN (1, 2, .) THEN CURRENTLY_DELQ = "X";
RUN;
...
PROC SQL;
	CREATE TABLE WORK.PAY_EQUIV AS
	SELECT l.BrAcctNo,
		   l.RegPyAmt, 
		   SUM(p.tramt) AS Payments,
		   CASE WHEN SUM(p.tramt) >= l.RegPyAmt * 1 
			THEN 'TRUE'
			ELSE 'FALSE'
	   		END AS ONE_PMNT_Flag,
		   CASE WHEN SUM(p.tramt) >= l.RegPyAmt * 2 
			THEN 'TRUE'
			ELSE 'FALSE'
	   		END AS TWO_PMNT_Flag,
	   	   CASE
			WHEN SUM(p.tramt) >= l.RegPyAmt * 3 
			THEN 'TRUE'
			ELSE 'FALSE'
	   		END AS THREE_PMNT_Flag,
	   	   CASE
			WHEN SUM(p.tramt) >= l.RegPyAmt * 4 
			THEN 'TRUE'
			ELSE 'FALSE'
	   		END AS FOUR_PMNT_Flag
	FROM DW.Payment p INNER JOIN 
		 DW.Loan l ON p.BrAcctNo = l.BrAcctNo
	WHERE p.TrCD in ('PA','PY','RV','PP','PE')
	GROUP BY l.BrAcctNo, 
		 	 l.RegPyAmt;
QUIT;

proc sort 
	data = PAY_EQUIV nodupkey out = PAY_EQUIV;
	by BrAcctNo;
RUN;

proc sort 
	data = MERGED_L_B2 out = MERGED_L_B2;
	by BrAcctNo;
RUN;

PROC SQL;
	CREATE TABLE WORK.MERGED_L_B2 AS 
	SELECT *
	FROM WORK.MERGED_L_B2 t1
   		LEFT JOIN WORK.PAY_EQUIV t2 
			ON t1.BrAcctNo = t2.BrAcctNo;
QUIT;
...
DATA MERGED_L_B2;
	...
	IF TWO_PMNT_Flag NE "TRUE" AND CLASSTRANSLATION NE 'Checks' 
		THEN LESSTHAN2_FLAG = "X";
	IF ONE_PMNT_Flag NE "TRUE" AND CLASSTRANSLATION = 'Checks' 
		THEN LESSTHAN2_FLAG = "X";
	IF TWO_PMNT_Flag = "TRUE" & _9S < 10 THEN LESSTHAN2_FLAG = "";
	IF FOUR_PMNT_Flag NE "TRUE" THEN LESSTHAN4_FLAG = "X";
	*IF LOANDATE > "&_4MO" THEN LESSTHAN4_FLAG = "X";
	
	*** pmt_days calculation wINs over conprofile ---------------- ***;
	IF TWO_PMNT_Flag = "TRUE" & _9S > 10 AND CLASSTRANSLATION NE 'Checks' 
		THEN LESSTHAN2_FLAG = "";
	IF ONE_PMNT_Flag = "TRUE" & _9S > 10 AND CLASSTRANSLATION = 'Checks' 
		THEN LESSTHAN2_FLAG = "";
...
DATA MERGED_L_B2;
	SET MERGED_L_B2;
	IF status_code_no IN (6, 10, 11, 13, 16, 17, 19, 21, 22, 23, 24, 
						  40, 41, 42, 43, 44, 45, 46, 47, 49, 50, 51, 
						  52, 53, 9001, 9002, 9003, 9004, 9005, 9006, 
						  9007, 9008, 9009, 9010, 9011, 9012, 9013, 
						  9014, 9015, 9016, 9018, 9019, 9020, 9021, 
						  9022, 9023, 9032, 9033, 9034, 9036, 9037, 
						  9039, 9040, 9041, 10000, 10001, 10002) 
		THEN STATFL_FLAG = "X";
RUN;
...
PROC EXPORT 
	DATA = DEDUPED 
	    OUTFILE = '\\mktg-app01\E\cepps\CAD\Reports\2022_05\PB_HOTLIST_2022_flagged_05022022.txt' 
		DBMS = TAB;
RUN;
...
DATA FINALL_OFFERS_PREP;
	...
	sl_pq_amount = 0;
	ll_pq_amount = 0;
	IF Available_Credit > Payoff_Amount * 0.10
		THEN availcredit_curbal_test = "X";
	IF Available_Credit >= 100
		THEN availcredit_test = "X";
	IF Payoff_Amount > 50
		THEN curbal_test = "X";
	IF recent4to6 = .
		THEN recent4to6 = 0;
	IF largeapprovedamount = .
		THEN largeapprovedamount = 0;
RUN;

DATA FINALL_OFFERS_PR;
	SET FINALL_OFFERS_PREP;
	IF classtranslation IN ('Small' 'Retail') AND
	   availcredit_curbal_test = "X" and
	   availcredit_test = "X" and
	   curbal_test = "X" and
	   LESSTHAN4_FLAG = "" and
	   Recent4to6 < 1
		then pr_offer = "X";
	...
	IF ownst IN ('GA' 'OK') and
	   pr_offer = "X"
		then pr_amount = 1400 - Payoff_Amount;

	IF ownst = 'TX' and
	   pr_offer = "X" and
	   Available_Credit - Payoff_Amount >= 2100
		then pr_amount = 2300 - Payoff_Amount;
	ELSE IF ownst = 'TX' and
	   pr_offer = "X" and
	   1420 <= Available_Credit - Payoff_Amount < 2100
		then pr_amount = 1320 - Payoff_Amount;

	IF ownst = 'AL' and
	   pr_offer = "X" and
	   Available_Credit >= 1900
		then pr_amount = 2300 - Payoff_Amount;
	ELSE IF ownst = 'AL' and
	   pr_offer = "X" and
	   1500 <= Available_Credit - Payoff_Amount < 1900
		then pr_amount = 1400 - Payoff_Amount;

		IF pr_amount > 1000 
		then pr_amount = 1000;
	IF pr_amount + Payoff_Amount > 2300
		then pr_amount = 2300 - Payoff_Amount;
RUN;

DATA FINALL_OFFERS_SL_PQ;
	SET FINALL_OFFERS_PR;
	IF classtranslation IN ('Small' 'Retail') AND
	   LESSTHAN4_FLAG = ""
		then sl_pq_offer = "X";
	...
	IF ownst IN ('GA' 'OK' 'AL') and
	   sl_pq_offer = "X"
		then sl_pq_amount = 1400 - Payoff_Amount;

	IF ownst IN ('TN') and
	   sl_pq_offer = "X"
		then sl_pq_amount = 1900 - Payoff_Amount;

	IF ownst IN ('TX') and
	   sl_pq_offer = "X"
		then sl_pq_amount = 1420 - Payoff_Amount;
	IF ownst = 'TX' and
	   sl_pq_offer = "X" and
	   Available_Credit + Payoff_Amount >= 2100
		then sl_pq_amount = 2300 - Payoff_Amount;
	IF ownst = 'TX' and
	   sl_pq_offer = "X" and
	   1420 <= Available_Credit + Payoff_Amount < 2100
		then sl_pq_amount = 2150 - Payoff_Amount;
	IF ownst = 'TX' and
	   sl_pq_offer = "X" and
	   Available_Credit + Payoff_Amount < 1420
		then sl_pq_amount = 1320 - Payoff_Amount;
	...
	*/
	...
	IF ownst = 'AL' and
	   sl_pq_offer = "X" and
	   Available_Credit + Payoff_Amount > 2150
		then sl_pq_amount = 2300 - Payoff_Amount;
	IF ownst = 'AL' and
	   sl_pq_offer = "X" and
	   1500 <= Available_Credit + Payoff_Amount <= 2150
		then sl_pq_amount = 2150 - Payoff_Amount;
	IF ownst = 'AL' and
	   sl_pq_offer = "X" and
	   Available_Credit + Payoff_Amount < 1500
		then sl_pq_amount = 1400 - Payoff_Amount;
	/*
	IF ownst = 'AL' and
	   sl_pq_offer = "X" and
	   Available_Credit + Payoff_Amount >= 1900
		then sl_pq_amount = 2150 - Payoff_Amount;
	ELSE IF ownst = 'AL' and
	   sl_pq_offer = "X" and
	   1500 <= Available_Credit + Payoff_Amount < 1900
...
DATA FINALL_OFFERS_LL_PQ;
	SET FINALL_OFFERS_SL_PQ;
	/*
	IF sl_pq_offer = "X" and
	...
	*/
	...
	IF ll_pq_offer = "X" and
	   Available_Credit + Payoff_Amount >= 7000
	   	then ll_pq_amount = 7000-Payoff_Amount;
RUN;
...
DATA FINALL_OFFERS_BEST;
	...
	*IF OFFER_AMOUNT < 50
	...
	IF PMT_DAYS = . THEN offer_type = "Branch ITA";
...
PROC EXPORT
	DATA = FINAL 
	 /* OUTFILE = '\\mktg-app01\E\Production\2018\CAD_BTS_2018\August_BTS_2018_final_06082018.txt' */
		OUTFILE = '\\mktg-app01\E\cepps\CAD\Reports\2022_05\PB_HOTLIST_2022_final_05022022.txt'
		REPLACE DBMS = TAB;
 RUN;
...
PROC EXPORT
	DATA = FINAL 
	 /* OUTFILE = '\\mktg-app01\E\Production\2018\CAD_BTS_2018\August_BTS_2018_final_06082018.txt' */
		OUTFILE = '\\mktg-app01\E\cepps\CAD\Reports\2022_05\PB_HOTLIST_2022_final_05022022.xlsx'
		REPLACE DBMS = XLSX;
 RUN;
...
Delete if Offer Amount or Payoff Amount < $50,
Delete if Partial Charge Off,
...
DATA _NULL_;
	SET FINALMLA;
	FILE "\\mktg-app01\E\Production\MLA\MLA-INput files TO WEBSITE\PB_HOTLIST_20210502.txt";
	PUT @ 1 "Social Security Number (SSN)"n 
		@ 10 "Date of Birth"n 
		@ 18 "Last NAME"n 
		@ 44 "First NAME"n 
...
*/
filename mla1 
	"\\mktg-app01\E\Production\MLA\MLA-Output files FROM WEBSITE\MLA_5_12_PB_HOTLIST_20220502p1.txt";
...
filename mla2 
	"\\mktg-app01\E\Production\MLA\MLA-Output files FROM WEBSITE\MLA_5_12_PB_HOTLIST_20220502p2.txt";
...
PROC EXPORT 
	DATA = FINALHH2 
	 /* OUTFILE = '\\mktg-app01\E\cepps\CAD\Reports\05_2018\August_CAD_BTS_2018_finalHH_05012018.txt' */
		OUTFILE = '\\mktg-app01\E\cepps\CAD\Reports\2022_05\PB_HOTLIST_2022_finalHH_05022022.txt'
		DBMS = DLM;
	DELIMITER = ",";
RUN;
...
PROC EXPORT
	DATA = FINALEC 
	 /* OUTFILE = '\\mktg-app01\E\cepps\CAD\Reports\05_2018\August_CAD_BTS_2018_final_EC_05012018.txt' */
		OUTFILE = '\\mktg-app01\E\cepps\CAD\Reports\2022_05\PB_HOTLIST_2022_final_EC_05022022.txt'
		DBMS = DLM;
	DELIMITER = ",";
RUN;
...
PROC EXPORT
	DATA = FINALEC 
	 /* OUTFILE = '\\rmc.local\dfsroot\Dept\MarketINg\2018 Programs\1) Direct Mail Programs\2018 CAD Programs\May 2018 CAD\August_CAD_BTS_2018_final_EC_05012018.xlsx' */
		OUTFILE = '\\mktg-app01\E\cepps\CAD\Reports\2022_05\PB_HOTLIST_2022_final_EC_05022022.xlsx'
	DBMS = XLSX;
RUN;
...
PROC EXPORT
	DATA = FINALEC2
	 /* OUTFILE = '\\rmc.local\dfsroot\Dept\MarketINg\2018 Programs\1) Direct Mail Programs\2018 CAD Programs\May 2018 CAD\August_CAD_BTS_2018_final_EC2_05012018.xlsx' */
		OUTFILE = '\\mktg-app01\E\cepps\CAD\Reports\2022_05\PB_HOTLIST_2022_final_preapproved_05022022.xlsx'
		DBMS = XLSX;
RUN;
...
PROC EXPORT
	DATA = FINALEC2 
	 /* OUTFILE = '\\rmc.local\dfsroot\Dept\MarketINg\2018 Programs\1) Direct Mail Programs\2018 CAD Programs\May 2018 CAD\August_CAD_BTS_2018_final_EC2_05012018.txt' */
		OUTFILE = '\\mktg-app01\E\cepps\CAD\Reports\2022_05\PB_HOTLIST_2022_final_preapproved_05022022.txt'
		DBMS = DLM;
	DELIMITER = ",";
RUN;
...
PROC EXPORT
	DATA = FINAL_PREQUAL 
	 /* OUTFILE = '\\rmc.local\dfsroot\Dept\MarketINg\2018 Programs\1) Direct Mail Programs\2018 CAD Programs\May 2018 CAD\August_CAD_BTS_2018_final_ITA_05012018.xlsx' */
		OUTFILE = '\\mktg-app01\E\cepps\CAD\Reports\2022_05\PB_HOTLIST_2022_final_prequalified_05022022.xlsx'
	DBMS = XLSX;
RUN;
...
PROC EXPORT
	DATA = FINAL_PREQUAL 
	 /* OUTFILE = '\\rmc.local\dfsroot\Dept\MarketINg\2018 Programs\1) Direct Mail Programs\2018 CAD Programs\May 2018 CAD\August_CAD_BTS_2018_final_EC2_05012018.txt' */
		OUTFILE = '\\mktg-app01\E\cepps\CAD\Reports\2022_05\PB_HOTLIST_2022_final_prequalified_05022022.txt'
		DBMS = DLM;
	DELIMITER = ",";
RUN;
...
PROC EXPORT
	DATA = FINAL_ITA 
	 /* OUTFILE = '\\rmc.local\dfsroot\Dept\MarketINg\2018 Programs\1) Direct Mail Programs\2018 CAD Programs\May 2018 CAD\August_CAD_BTS_2018_final_ITA_05012018.xlsx' */
		OUTFILE = '\\mktg-app01\E\cepps\CAD\Reports\2022_05\PB_HOTLIST_2022_final_ITA_05022022.xlsx'
	DBMS = XLSX;
RUN;
...
PROC EXPORT
	DATA = FINAL_NAC 
	 /* OUTFILE = '\\rmc.local\dfsroot\Dept\MarketINg\2018 Programs\1) Direct Mail Programs\2018 CAD Programs\May 2018 CAD\August_CAD_BTS_2018_final_ITA_05012018.xlsx' */
		OUTFILE = '\\mktg-app01\E\cepps\CAD\Reports\2022_05\PB_HOTLIST_2022_final_NAC_05022022.xlsx'
	DBMS = XLSX;
RUN;
...
PROC EXPORT
	DATA = FINAL_NAC 
	 /* OUTFILE = '\\rmc.local\dfsroot\Dept\MarketINg\2018 Programs\1) Direct Mail Programs\2018 CAD Programs\May 2018 CAD\August_CAD_BTS_2018_final_ITA_05012018.xlsx' */
		OUTFILE = '\\mktg-app01\E\cepps\CAD\Reports\2022_05\PB_HOTLIST_2022_final_NAC_OFFER_05022022.xlsx'
	DBMS = XLSX;
RUN;
